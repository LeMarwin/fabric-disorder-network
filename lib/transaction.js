...
class Transaction {
	...
	async submitUnordered(...args) {
		...
		// Submit the endorsed transaction to the peers
		const responses = await channel.sendUnorderedTransaction({
			proposalResponses: validResponses,
			proposal
		});

		const finResults = responses.filter(resp => {
			let flag = true;
